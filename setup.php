...
if (!is_https()) {
    echo "Sorry - Setup can't run - You are not using HTTPS. Please switch to HTTPS and run setup again.";
	exit();
}
...
<br><br><br><b>CONGRATULATIONS!</b><br><br>Update Successful.
</div>
EOT;


	@unlink("setup.php");
	@unlink("setupdeploy.php");
	@unlink("SETUP_imgup.php");
	echo $updatemessage;
	exit();
}
...
$l1 = substr(implode('', array_map(function () {return '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'[random_int(0, 61)];}, range(1, 20))), 0, 20);
$configout=<<<EOT
<?php
\$version='1.0';
\$sitename='$sitename';
...
EOT;
	...
	if (!file_exists('configpacks.php')) {
...
EOT;
	...
	$urlhere = $protocol . $domainName . $scriptPath . '/';
...
#setup label, #setup input, #setup textarea, #setup select, #setup button {
  width: 100%;
  margin: 10px 0;
}
#setup input, #setup textarea, #setup select, #setup button {
  padding: 10px;
  border:none;
...
<body>
    ...
    <form id="setup" method="post">
		<div style="text-align:center; margin-top:10px;"><h2 style="font-size:18px" >Setup - Please complete the form / upload your logo and press GO.
		<br><br>

		</h2></div>
		<br>

		Upload Logo (PNG only - recommended size 500px X 120px)
		<div style="display: flex; align-items: center;">
			<input type="file" id="fileUpload" name="fileUpload" style="display: none;" />
			<button onclick="document.getElementById('fileUpload').click();">Upload</button>
			<img id="preview" style="cursor:pointer; margin-left:10px; height: 40px; transition: all 0.5s;" src="#" alt="Image preview..." hidden/>
		</div>
		...
		Admin Username
		<input type="text" name="adminuser" value="admin" required>
		Admin Password
		<input type="text" name="adminpassword" value="" required>
		Link to your terms (leave blank if you have none)
		<input type="text" name="termslink" placeholder="https://mytermslink.com">
		Link to your support desk (leave blank if you have none)
		<input type="text" name="supportlink" placeholder="https://mysupportlink.com">
		Support email address (leave blank if you have none)
		<input type="text" name="supportemail" placeholder="support@mysupport.com">
